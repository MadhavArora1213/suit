...
export default function CartPage({ cart, updateCartQty, removeFromCart, setView }) {
  ...
  return (
    <motion.div 
      ...
      exit={{ opacity: 0, y: -20 }}
      className="max-w-[1200px] mx-auto px-6 pt-32 pb-24 min-h-[80vh] text-[#1A0008]"
      style={{ fontFamily: "'DM Sans', sans-serif" }}
    >
      {/* Back to Home Link */}
      <button 
        onClick={() => window.location.href = '/'} 
        className="flex items-center gap-2 text-xs tracking-widest text-[#D4AF37] uppercase font-bold mb-8 hover:text-[#1A0008] transition-colors cursor-pointer"
      >
        <ArrowLeft size={14} />
        <span>Back to Shopping</span>
      </button>

      <h1 className="text-4xl md:text-5xl font-light mb-12 tracking-tight text-[#1A0008]" style={{ fontFamily: "'Cormorant Garamond', serif" }}>
        Shopping Bag
      </h1>

      {cart.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-center border border-[#D4AF37]/15 bg-white p-10 rounded">
          <div className="w-20 h-20 rounded-full border border-[#D4AF37]/20 flex items-center justify-center text-[#D4AF37]/40 mb-6">
          ...
          <button 
            onClick={() => window.location.href = '/'}
            className="bg-[#D4AF37] hover:bg-[#9A8268] text-[#FAF9F6] px-8 py-4 text-xs font-bold tracking-widest uppercase transition-colors shadow-lg cursor-pointer"
          >
            EXPLORE COLLECTIONS
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 items-start">
          ...
          <div className="lg:col-span-2 space-y-6">
            {cart.map((item) => (
              <div key={`${item.id}-${item.size}`} className="flex flex-col sm:flex-row gap-6 p-6 border border-[#D4AF37]/10 bg-white shadow-sm relative">
                <div className="w-28 h-36 bg-[#E8DDD0] overflow-hidden flex-shrink-0 border border-[#D4AF37]/10">
                ...
                <div className="flex-1 flex flex-col justify-between">
                  <div className="space-y-1">
                    ...
                    <span className="text-xs text-[#6B6B6B] block">
                      Size: <span className="text-[#1A0008] font-semibold">{item.size}</span>
                    </span>
                  </div>
                  ...
                  <div className="flex items-center justify-between mt-6">
                    ...
                    <span className="text-xl font-light text-[#D4AF37]" style={{ fontFamily: "'Cormorant Garamond', serif" }}>
                      {item.price}
                    </span>
                  </div>
                </div>
          ...
          {/* Cart Summary Card */}
          <div className="border border-[#D4AF37]/15 bg-white p-8 space-y-6 shadow-md">
            ...
            <div className="space-y-3">
              ...
              <div className="flex justify-between text-xs text-[#6B6B6B] uppercase tracking-wider font-semibold">
                <span>Shipping</span>
                <span className="text-[#1A0008]">FREE</span>
              </div>
              <div className="flex justify-between text-xs text-[#6B6B6B] uppercase tracking-wider font-semibold">
                <span>Estimated Taxes</span>
                <span className="text-[#1A0008]">Calculated next</span>
              </div>
            </div>
            ...
            <div className="border-t border-[#D4AF37]/10 pt-4 flex justify-between items-center">
              ...
              <span className="text-2xl text-[#1A0008] font-light" style={{ fontFamily: "'Cormorant Garamond', serif" }}>
