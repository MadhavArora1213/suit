...
export default function LoginSignup({ setView, onLoginSuccess }) {
    ...
    fetch('https://api.brevo.com/v3/smtp/email', {
      ...
      body: JSON.stringify({
        ...
        htmlContent: `
          ...
          <head>
          <style>
            body { font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; background-color: #FAF9F6; margin: 0; padding: 40px 20px; color: #1A0008; }
            .container { max-width: 500px; margin: 0 auto; background-color: #ffffff; padding: 40px; border-radius: 8px; box-shadow: 0 10px 30px rgba(0,0,0,0.03); text-align: center; border-top: 4px solid #D4AF37; }
            .logo { font-size: 20px; font-weight: 700; letter-spacing: 0.2em; color: #1A0008; margin-bottom: 30px; text-transform: uppercase; }
            .title { font-size: 22px; font-weight: 300; margin-bottom: 15px; color: #1A0008; }
            .text { color: #666666; font-size: 14px; line-height: 1.6; margin-bottom: 25px; }
            .otp-box { background-color: #FAF9F6; border: 1px solid #EAEAEA; padding: 24px; font-size: 38px; font-weight: 600; letter-spacing: 0.15em; color: #D4AF37; margin: 30px 0; border-radius: 6px; }
  ...
  return (
    <div className="min-h-screen bg-[#FAF9F6] relative overflow-hidden flex flex-col">
      ...
      {/* Centered content */}
      <div className="flex-1 flex items-center justify-center px-5 sm:px-6 pt-24 md:pt-32 pb-16 md:pb-20">
          ...
          {errors.form && (
            ...
            >
              <motion.div
                ...
                className="relative w-full max-w-[280px] sm:max-w-[360px] aspect-square flex items-center justify-center bg-transparent"
            ...
            {!otpSent ? (
              ...
              >
                ...
                <div className="text-center mb-8 md:mb-12">
                  <h1 className="text-[36px] sm:text-[44px] md:text-[56px] font-light leading-none mb-3 md:mb-4" style={{ fontFamily: "'Cormorant Garamond', serif" }}>
                    {mode === 'login' ? 'Welcome Back' : 'Create Account'}
                  </h1>
                  <p className="text-[11px] sm:text-[13px] text-[#1A0008]/30 font-light">
                    {mode === 'login'
                ...
                <button
                  ...
                  disabled={loading}
                  className="w-full border border-[#1A0008]/8 rounded-full py-3 sm:py-3.5 flex items-center justify-center gap-3 hover:border-[#D4AF37]/40 hover:bg-white transition-all duration-300 cursor-pointer mb-5 sm:mb-8"
                >
                  ...
                  <span className="text-[12px] text-[#1A0008]/50 font-medium">Continue with Google</span>
                </button>

                {/* Divider */}
                <div className="flex items-center gap-4 mb-5 sm:mb-8">
                  <div className="flex-1 h-px bg-[#1A0008]/6" />
                  <span className="text-[9px] tracking-[0.3em] text-[#1A0008]/15 uppercase">or</span>
                  <div className="flex-1 h-px bg-[#1A0008]/6" />
                </div>

                {/* Form */}
                <form onSubmit={mode === 'login' ? handleLogin : handleSignup} className="space-y-4 sm:space-y-5">
                      ...
                      <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} transition={{ duration: 0.25 }}>
                        <div className="relative">
                          <input
                            ...
                            className="w-full bg-transparent border-b border-[#1A0008]/8 focus:border-[#D4AF37] outline-none py-3 sm:py-3.5 text-[13px] sm:text-[14px] text-[#1A0008] placeholder:text-[#1A0008]/20 font-light transition-colors"
                          />
                    ...
                    {mode === 'signup' && (
                      <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} transition={{ duration: 0.25 }}>
                        <div className="relative flex gap-3">
                          ...
                          <div className="w-[85px] sm:w-[100px] shrink-0 border-b border-[#1A0008]/8 focus-within:border-[#D4AF37] transition-colors relative">
                            <select 
                              value={form.countryCode} 
                              onChange={e => update('countryCode', e.target.value)}
                              className="w-full bg-transparent outline-none py-3 sm:py-3.5 text-[13px] sm:text-[14px] text-[#1A0008] appearance-none cursor-pointer pr-6"
                            >
                            ...
                            <div className="absolute right-0 top-1/2 -translate-y-1/2 pointer-events-none text-[#1A0008]/40 text-[10px]">
                              ▼
                            </div>
                          </div>
                  ...
                  <div className="relative mb-6">
                    <input
                      ...
                      className="w-full bg-transparent border-b border-[#1A0008]/8 focus:border-[#D4AF37] outline-none py-3 sm:py-3.5 pr-10 text-[13px] sm:text-[14px] text-[#1A0008] placeholder:text-[#1A0008]/20 font-light transition-colors"
                    />
                  ...
                  {mode === 'login' && (
                    <div className="flex justify-end -mt-1">
                      <button 
                        ...
                        disabled={loading}
                        className="text-[11px] text-[#1A0008]/25 hover:text-[#D4AF37] transition-colors cursor-pointer"
                      >
                        Forgot Password?
                      </button>
                    </div>
                  ...
                  <motion.button
                    ...
                    whileTap={{ scale: 0.99 }}
                    className="w-full bg-[#1A0008] text-white py-3.5 sm:py-4 rounded-full text-[10px] tracking-[0.3em] font-bold uppercase hover:bg-[#D4AF37] disabled:opacity-40 transition-all duration-500 cursor-pointer mt-4 sm:mt-6 flex items-center justify-center gap-2"
                  >
                    {loading ? (
                ...
                {/* Terms */}
                {mode === 'signup' && (
                  <p className="text-[10px] text-[#1A0008]/18 text-center mt-6 leading-relaxed">
                    By signing up, you agree to our{' '}
                    <button onClick={() => setView('privacy')} className="text-[#D4AF37]/50 hover:text-[#D4AF37] cursor-pointer underline underline-offset-2">Privacy Policy</button>.
                  </p>
                )}

                {/* Toggle */}
                <div className="mt-6 sm:mt-10 pt-6 sm:pt-8 text-center">
                  <p className="text-[12px] text-[#1A0008]/25">
              ...
              >
                <div className="text-center mb-8 md:mb-12">
                  ...
                  <p className="text-[11px] sm:text-[13px] text-[#1A0008]/30 font-light">
                    ...
                    <span className="text-[#1A0008]/50">{form.email}</span>
                  </p>
                </div>
                ...
                <div className="flex items-center justify-center gap-2 sm:gap-3 mb-6 sm:mb-10">
                  {otpCode.map((digit, i) => (
                    <motion.input
                      ...
                      className="w-11 sm:w-14 md:w-16 h-[56px] sm:h-[64px] md:h-[72px] text-center text-[24px] sm:text-[28px] md:text-[32px] font-light border-b-2 border-[#1A0008]/8 focus:border-[#D4AF37] bg-transparent outline-none transition-colors duration-300"
                ...
                <motion.button
                  ...
                  whileHover={{ scale: 1.01 }}
                  whileTap={{ scale: 0.99 }}
                  className="w-full bg-[#1A0008] text-white py-4 rounded-full text-[10px] tracking-[0.3em] font-bold uppercase hover:bg-[#D4AF37] disabled:opacity-30 transition-all duration-500 cursor-pointer flex items-center justify-center gap-2"
                >
                  {loading ? (
                    <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />
                  ) : (
                ...
                <div className="text-center mt-8 space-y-3">
                  {timer > 0 ? (
                    <p className="text-[12px] text-[#1A0008]/20">
                      Resend in <span className="text-[#1A0008]/40 tabular-nums">{timer}s</span>
                    </p>
                  ) : (
                    <button onClick={handleSignup} className="text-[12px] text-[#D4AF37] hover:underline cursor-pointer">
                      Resend Code
                    </button>
                  )}
                  <button
                    onClick={() => { setOtpSent(false); setOtpCode(['', '', '', '', '', '']); }}
                    className="block w-full text-[11px] text-[#1A0008]/20 hover:text-[#D4AF37] transition-colors cursor-pointer"
                  >
                    Change Email
                  </button>
                </div>
      ...
      {/* Bottom bar */}
      <div className="absolute bottom-0 left-0 right-0 flex items-center justify-between px-5 sm:px-6 md:px-10 py-4 sm:py-5">
        <p className="text-[10px] text-[#1A0008]/15 tracking-wide">&copy; 2026 Gurnaaz</p>
        <button onClick={() => setView('privacy')} className="text-[10px] text-[#1A0008]/15 hover:text-[#D4AF37] transition-colors cursor-pointer tracking-wide">
          Privacy
        </button>
      </div>
    </div>
