...
export default function Navbar({ cart = [], removeFromCart, updateCartQty, favorites = {}, toggleFavorite, addToCart }) {
  ...
  return (
    ...
    >
      {/* Announcement Bar */}
      <div className="bg-[#111111] text-[#FAF9F6] py-2 px-6 text-center text-[9px] sm:text-[10px] tracking-[0.22em] font-semibold flex items-center justify-center gap-4 uppercase">
      ...
      <div className={`transition-all duration-500 ${scrolled
        ? 'bg-[#111111]/95 backdrop-blur-xl border-b border-[#BCA58A]/10 shadow-lg'
        : 'bg-transparent'
      }`}>
        <div className="max-w-[1600px] mx-auto px-6 md:px-14 py-4">
          <div className="flex items-center justify-between">
            ...
            <motion.a href="#" whileHover={{ scale: 1.02 }} className="flex flex-col items-start cursor-pointer group">
              <span
                className="text-[26px] md:text-[30px] font-semibold tracking-[0.18em] text-[#FAF9F6] group-hover:text-[#BCA58A] transition-colors duration-400"
            ...
            <div className="hidden md:flex items-center gap-10">
              {['HOME', 'SHOP', 'COLLECTIONS', 'BOUTIQUES', 'ABOUT US'].map((item, i) => (
                <a key={i} href="#"
                  className="relative text-[11px] tracking-[0.18em] text-[#FAF9F6]/70 hover:text-[#FAF9F6] transition-colors duration-300 py-2 group"
            ...
            <div className="flex items-center gap-6 md:gap-7">
              <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.95 }}
                ...
                className="text-[#FAF9F6]/70 hover:text-[#BCA58A] transition-colors cursor-pointer">
                <Search size={18} />
              </motion.button>
              ...
              <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.95 }}
                ...
                className="text-[#FAF9F6]/70 hover:text-[#BCA58A] transition-colors relative cursor-pointer">
                <Heart size={18} />
                {favoriteCount > 0 && (
                  <motion.span initial={{ scale: 0 }} animate={{ scale: 1 }}
                    className="absolute -top-1.5 -right-1.5 bg-[#BCA58A] text-[#111111] text-[9px] font-bold rounded-full w-4 h-4 flex items-center justify-center">
                    {favoriteCount}
                  </motion.span>
                )}
              </motion.button>
              ...
              <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.95 }}
                ...
                {cartItemCount > 0 && (
                  <motion.span initial={{ scale: 0 }} animate={{ scale: 1 }}
                    className="absolute -top-1.5 -right-1.5 bg-[#FAF9F6] text-[#111111] text-[9px] font-bold rounded-full w-4 h-4 flex items-center justify-center">
                    {cartItemCount}
                  </motion.span>
                )}
              </motion.button>
              ...
              <motion.button whileHover={{ scale: 1.1 }}
                ...
                <span className={`w-5 h-px bg-[#FAF9F6] transition-all duration-300 ${isOpen ? 'rotate-45 translate-y-1.5' : ''}`} />
                <span className={`w-5 h-px bg-[#FAF9F6] transition-all duration-300 ${isOpen ? 'opacity-0' : ''}`} />
                <span className={`w-5 h-px bg-[#FAF9F6] transition-all duration-300 ${isOpen ? '-rotate-45 -translate-y-1.5' : ''}`} />
              </motion.button>
            </div>
          </div>
                ...
                <div className="border-t border-[#BCA58A]/15 pt-4 pb-3">
                  <div className="flex items-center gap-3">
                    <input type="text" placeholder="Search for suits, anarkalis, dupattas…"
                      ...
                      className="w-full bg-[#1E1E1E] text-[#FAF9F6] placeholder-[#6B6B6B] border border-[#BCA58A]/20 rounded-none px-5 py-3 text-sm focus:outline-none focus:border-[#BCA58A] transition-colors"
                      style={{ fontFamily: "'DM Sans', sans-serif" }} autoFocus />
                    <motion.button whileHover={{ scale: 1.05 }} onClick={() => setSearchOpen(false)}
                      className="text-[#6B6B6B] p-1.5 hover:text-[#FAF9F6] cursor-pointer">
                      <X size={20} />
                    </motion.button>
                  </div>
                  <div className="flex flex-wrap items-center gap-2 mt-3 pl-1">
                    ...
                    {suggestions.map((sug) => (
                      <button key={sug} onClick={() => setSearchQuery(sug)}
                        className="text-[10px] font-medium bg-[#1E1E1E] border border-[#BCA58A]/20 text-[#FAF9F6]/70 hover:border-[#BCA58A] hover:text-[#BCA58A] px-3 py-1 transition-colors cursor-pointer">
                        {sug}
                      </button>
                    ))}
                  </div>
          ...
          <AnimatePresence>
            {isOpen && (
              <motion.div initial={{ opacity: 0, x: '100%' }} animate={{ opacity: 1, x: 0 }}
                ...
                className="fixed inset-y-0 right-0 w-64 bg-[#111111] border-l border-[#BCA58A]/10 shadow-2xl z-50 p-8 flex flex-col gap-6 md:hidden">
                <div className="flex justify-between items-center border-b border-[#BCA58A]/15 pb-4">
                  <span className="text-lg tracking-[0.2em] text-[#FAF9F6]" style={{ fontFamily: "'Cormorant Garamond', serif", fontStyle: 'italic' }}>MENU</span>
                  <button onClick={() => setIsOpen(false)} className="text-[#6B6B6B] hover:text-[#FAF9F6] cursor-pointer"><X size={20} /></button>
                </div>
                <div className="flex flex-col gap-5">
                  {['HOME', 'SHOP', 'COLLECTIONS', 'BOUTIQUES', 'ABOUT US'].map((item, i) => (
                    <a key={i} href="#" onClick={() => setIsOpen(false)}
                      className="text-[11px] tracking-[0.2em] text-[#FAF9F6]/60 hover:text-[#BCA58A] transition-colors py-1 border-b border-[#BCA58A]/10">
                      {item}
                    </a>
                  ))}
                </div>
          ...
          <div className="fixed inset-0 z-50 overflow-hidden">
            ...
            <div className="absolute inset-y-0 right-0 max-w-full flex">
              <motion.div initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }}
                ...
                className="w-screen max-w-md bg-[#111111] border-l border-[#BCA58A]/10 shadow-2xl flex flex-col">
                <div className="p-6 border-b border-[#BCA58A]/10 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <ShoppingBag size={18} className="text-[#BCA58A]" />
                    <span className="text-base tracking-[0.1em] text-[#FAF9F6]" style={{ fontFamily: "'Cormorant Garamond', serif" }}>SHOPPING BAG</span>
                    <span className="bg-[#BCA58A]/10 text-[#BCA58A] text-[9px] font-semibold px-2 py-0.5 rounded-full">{cartItemCount} items</span>
                  </div>
                  <button onClick={() => setCartOpen(false)} className="text-[#6B6B6B] hover:text-[#FAF9F6] cursor-pointer"><X size={20} /></button>
                </div>
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-6">
                  {cart.length === 0 ? (
                    <div className="h-full flex flex-col items-center justify-center text-center space-y-4">
                      ...
                      <h4 className="text-[#FAF9F6]/70 text-base" style={{ fontFamily: "'Cormorant Garamond', serif" }}>Your bag is empty</h4>
                      ...
                      <button onClick={() => setCartOpen(false)}
                        className="px-6 py-2.5 border border-[#BCA58A] text-[#BCA58A] text-[10px] font-semibold tracking-widest hover:bg-[#BCA58A] hover:text-[#111111] transition-colors cursor-pointer">
                        CONTINUE SHOPPING
                      </button>
                    </div>
                  ) : (
                    cart.map((item) => (
                      <div key={`${item.id}-${item.size}`} className="flex gap-4 pb-6 border-b border-[#BCA58A]/10 last:border-0">
                        <div className="w-20 h-24 overflow-hidden bg-[#1E1E1E] border border-[#BCA58A]/10 flex-shrink-0">
                          <img src={item.image} alt={item.name} className="w-full h-full object-cover object-top" />
                        </div>
                        <div className="flex-1 flex flex-col justify-between">
                          <div>
                            <div className="flex justify-between items-start gap-2">
                              <h4 className="text-xs font-medium text-[#FAF9F6] tracking-wide line-clamp-1">{item.name}</h4>
                              <button onClick={() => removeFromCart(item.id, item.size)}
                                className="text-[#6B6B6B] hover:text-[#111111] transition-colors cursor-pointer"><Trash2 size={13} /></button>
                            </div>
                            <span className="text-[10px] text-[#6B6B6B] block mt-0.5">Size: <span className="text-[#FAF9F6]">{item.size}</span></span>
                          </div>
                          <div className="flex items-center justify-between mt-2">
                            <div className="flex items-center border border-[#BCA58A]/20">
                              ...
                              <span className="text-xs font-medium px-2 text-[#FAF9F6]">{item.quantity}</span>
                              <button onClick={() => updateCartQty(item.id, item.size, item.quantity + 1)}
                ...
                {cart.length > 0 && (
                  <div className="p-6 bg-[#111111] border-t border-[#BCA58A]/10 space-y-4">
                    <div className="flex justify-between items-center">
                      <span className="text-[10px] tracking-widest text-[#6B6B6B] uppercase">Estimated Subtotal</span>
                      <span className="text-lg text-[#FAF9F6]" style={{ fontFamily: "'Cormorant Garamond', serif" }}>₹{getSubtotal().toLocaleString()}</span>
                    </div>
                    ...
                    <button onClick={handleCheckout}
                      className="w-full bg-[#BCA58A] hover:bg-[#BCA58A] text-[#111111] py-4 text-[10px] font-bold tracking-[0.25em] flex items-center justify-center gap-2.5 transition-colors shadow-lg cursor-pointer uppercase">
          ...
          <div className="fixed inset-0 z-50 overflow-hidden">
            ...
            <div className="absolute inset-y-0 right-0 max-w-full flex">
              <motion.div initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }}
                ...
                <div className="p-6 border-b border-[#BCA58A]/10 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <Heart size={18} className="text-[#111111] fill-[#111111]" />
                    <span className="text-base tracking-[0.1em] text-[#FAF9F6]" style={{ fontFamily: "'Cormorant Garamond', serif" }}>MY WISHLIST</span>
                    <span className="bg-[#111111]/15 text-[#111111] text-[9px] font-semibold px-2 py-0.5 rounded-full">{favoriteCount} items</span>
                  </div>
                  <button onClick={() => setWishlistOpen(false)} className="text-[#6B6B6B] hover:text-[#FAF9F6] cursor-pointer"><X size={20} /></button>
                </div>
                <div className="flex-1 overflow-y-auto p-6 space-y-6">
                  {favoriteItems.length === 0 ? (
                    <div className="h-full flex flex-col items-center justify-center text-center space-y-4">
                      <div className="w-16 h-16 rounded-full border border-[#111111]/20 flex items-center justify-center text-[#111111]/30">
                        <Heart size={28} className="stroke-[1.25]" />
                      </div>
                      <h4 className="text-[#FAF9F6]/70 text-base" style={{ fontFamily: "'Cormorant Garamond', serif" }}>Your wishlist is empty</h4>
                    ...
                    favoriteItems.map((item) => (
                      <div key={item.id} className="flex gap-4 pb-6 border-b border-[#BCA58A]/10 last:border-0">
                        ...
                        <div className="flex-1 flex flex-col justify-between text-left">
                          <div>
                            <div className="flex justify-between items-start gap-2">
                              <h4 className="text-xs font-medium text-[#FAF9F6]">{item.name}</h4>
                              <button onClick={() => toggleFavorite(item.id)} className="text-[#6B6B6B] hover:text-[#111111] cursor-pointer"><X size={14} /></button>
                            </div>
                            <span className="text-[9px] text-[#BCA58A] font-semibold uppercase tracking-wider block mt-1">{item.boutique} · Verified</span>
                            <span className="text-sm text-[#FAF9F6] block mt-1" style={{ fontFamily: "'Cormorant Garamond', serif" }}>{item.price}</span>
                          </div>
                          <button onClick={() => { addToCart(item, 'M'); alert(`Added ${item.name} to bag!`); }}
                            className="mt-3 bg-[#1E1E1E] border border-[#BCA58A]/20 hover:border-[#BCA58A] hover:bg-[#BCA58A] hover:text-[#111111] text-[#FAF9F6] py-2 text-[9px] font-semibold tracking-widest uppercase flex items-center justify-center gap-1.5 transition-all cursor-pointer">
